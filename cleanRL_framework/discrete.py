...
class CoppeliaSimEnv(gym.Env):
    def __init__(self):
        ...
        self.sim.loadScene('/home/binggwong/git/DQN_ball_balance/scenes/Hangbot_cube_2D.ttt') # Ubuntu
        ...
        self.attach_step = 1  # Define attach step threshold
        ...
        # Action space: discrete actions for curvature control
        # Let's use 5 discrete actions: [very_negative, negative, neutral, positive, very_positive]
        self.action_space = gym.spaces.Discrete(5)
        self.action_mapping = {
            0: -1.0,  # very negative curvature
            1: -0.5,   # negative curvature
            2: 0.0,     # neutral/straight
            3: 0.5,    # positive curvature
            4: 1.0    # very positive curvature
        }
    ...
    def step(self, action):
        # Take action: map discrete action to platform tilting
        # Action is now a discrete integer (0-4)
        ...
        if self.robot_state == 0:
            ...
            if self.transition_counter < self.transition_step:
                ...
                if self.magnet2_detect and y_distance < 0:
                    self.magnet1State = True
                    self.magnet2State = False
                elif self.magnet2_detect and self.cd_m2 > 10:
            ...
            if self.transition_counter < self.transition_step:
                ...
                if self.magnet1_detect and y_distance < 0:
                    self.magnet1State = False
                    self.magnet2State = True
                elif self.magnet1_detect and self.cd_m1 > 10:
                    self.magnet_counter = self.magnet_counter + 1
                if self.magnet_counter > self.attach_step:
                    self.robot_state = 0
                    self.magnet_counter = 0
        ...
        # Curvature Control
        # print("Action: ", action)
        # Convert discrete action to continuous curvature value
        self.Curv = self.action_mapping[action]
        ...
        next_state = np.array([self.Curv, magnet1_state, magnet2_state,  # Normalize curvature to [-1, 1]
    ...
    def compute_reward(self, state):
        ...
        if self.robot_state == 0:
            ...
            attach_reward += 10.0 if self.magnet2_detect and self.m2_position[1] > self.m1_position[1] else 0.0
            attach_reward -= 10.0 if self.magnet2_detect and self.m2_position[1] < self.m1_position[1] else 0.0
        if self.robot_state == 1:
            attach_reward = self.calculate_attach_point(self.m1_position, self.m2_position) * 10
            attach_reward += 10.0 if self.magnet1_detect and self.m1_position[1] > self.m2_position[1] else 0.0
            attach_reward -= 10.0 if self.magnet1_detect and self.m1_position[1] < self.m2_position[1] else 0.0
    ...
    def calculate_attach_point(self, magnet_position, fixate_magnet_position):
        ...
        reward = - (5*y_diff ** 2 + z_diff ** 2)  # Reward is negative distance from target position
        return reward
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        # Actor head for discrete actions
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            action_numpy = action.cpu().numpy().item()
